    ...
    public static function table(Table $table)
    {
        return $table
            ->columns([
                ...
                Columns\Text::make("email")->url(
                    fn($customer) => "mailto:{$customer->email}"
                ),
                Columns\Text::make("created_at"),
                // Columns\Text::make("id")
                //     ->label("Impersonate")
                //     ->getValueUsing($callback = fn($record) => "do one")
                //     ->url(fn($record) => route("impersonate", $record->id)),
                \App\Filament\Resources\Tables\Columns\Impersonate::make("id")->label(""),
            ])
            ->filters([
                //
            ]);
