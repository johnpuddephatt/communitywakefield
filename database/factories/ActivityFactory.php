<?php

namespace Database\Factories;

use Illuminate\Database\Eloquent\Factories\Factory;
use Illuminate\Support\Str;
use App\Models\Activity;
use App\Models\Team;
use App\Models\Subteam;
use App\Models\User;

class ActivityFactory extends Factory
{
    /**
     * The name of the factory's corresponding model.
     *
     * @var string
     */
    protected $model = Activity::class;

    /**
     * Define the model's default state.
     *
     * @return array
     */
    public function definition()
    {



        return [
            'subteam_id' => Subteam::factory(),
            'team_id' => function (array $activity) {
                return \App\Models\Subteam::find($activity['subteam_id'])->team->id;
            },
            'updated_by' => function (array $activity) {
                return \App\Models\Subteam::find($activity['subteam_id'])->team->owner->id;
            },
            'created_by' => function (array $activity) {
                return \App\Models\Subteam::find($activity['subteam_id'])->team->owner->id;
            },
            'display_until' => $this->faker->date(),
            'status' => $this->faker->randomElement(["Published","Draft"]),
            'title' => $this->faker->sentence(4),
            'slug' => $this->faker->slug,
            'content' => $this->faker->paragraphs(3, true),
            'phone' => $this->faker->phoneNumber,
            'email' => $this->faker->safeEmail,
            'from_home' => $this->faker->boolean,
            'address' => $this->faker->address,
            'address_ward' => $this->faker->word,
            'postcode' => $this->faker->postcode,
            'latitude' => $this->faker->latitude,
            'longitude' => $this->faker->longitude,
            'directions' => $this->faker->sentence(10),
            'times' => $this->faker->sentence(10),
            'minimum_age' => $this->faker->numberBetween(5, 21),
            'maximum_age' => $this->faker->numberBetween(12, 100),
            'cost' => $this->faker->sentence(10),
            'what_to_bring' => $this->faker->sentence(12),
            'booking_link' => $this->faker->url,
            'booking_instructions' => $this->faker->sentence(10),
        ];
    }
}
