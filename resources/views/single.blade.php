...
<div class="container container__row-reverse entry-single">
      <div class="entry-single--main">
          <a class="button button__secondary entry-single--enquire-button" href="{{ route(strtolower(get_class($entry)::$name) . '.enquire', ['entry' => $entry ])}}" >Enquire</a>
          ...
          <div class="entry-single--key-features">
            @if(isset($entry->cost))
                  <span class="entry-single--key-feature"><x-icon.cost /> {{ $entry->cost ? $entry->cost : 'Free' }}</span>
            @endif
            @if(isset($entry->hours))
                <span class="entry-single--key-feature"><x-icon.clock /> {{ $entry->hours ? $entry->hours . ' hrs / week' : 'Flexible hours' }}</span>
            @endif
            @if(isset($entry->start_date))
                <span class="entry-single--key-feature"><x-icon.calendar /> {{ $entry->date_range() }}</span>
            @endif
            @if($entry->address_ward && isset($entry->address['value']))
              <span class="entry-single--key-feature"><x-icon.map-marker /> {{ $entry->address_ward }} <a target="_blank" href="https://maps.google.com/?q={{ urlencode($entry->address['value']) }}">view map</a></span>
            @endif
          </div>
          ...
          @if($entry->deadline)
            <div class="entry-single--deadline alert alert__info">Deadline: <strong>{{ date("D jS F, Y", strtotime($entry->deadline)) }}</strong></div>
          @endif
          ...
          <div class="entry-single--other-features">
            ...
            @if(!empty($entry->accessibilities))
              <div>
                <h3>Accessibility</h3>
            ...
            @if(!empty($entry->minimum_age) && !empty($entry->maximum_age))
              <div>
                <h3>Age range</h3>
                {{ $entry->minimum_age }} – {{ $entry->maximum_age }}
              </div>
            @elseif(!empty($entry->minimum_age))
                  <div>
                    <h3>Minimum age</h3>
                    {{ $entry->minimum_age }}
                  </div>
            @elseif(!empty($entry->maximum_age))
                  <div>
                    <h3>Maximum age</h3>
                    {{ $entry->maximum_age }}
                  </div>
    ...
    <div class="entry-single--sidebar">
      ...
      @endif
      @if($entry->phone)
        <div class="entry-single--sidebar--phone"><x-icon.phone /> {{ $entry->phone }}</div>
      @endif
      @if($entry->email)
        <div class="entry-single--sidebar--email"><x-icon.email /> <a href="mailto:{{ $entry->email }}">{{ $entry->email }}</a></div>
      @endif
      {{-- <a class="button button__inverted" href="{{ route('entry.index') }}?{{ http_build_query(['organisation' => $entry->team->slug]) }}">See all opportunities with this organisation</a> --}}
    </div>
@endsection
