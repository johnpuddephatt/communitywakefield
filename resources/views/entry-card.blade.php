<div class="entry-card">
  <div class="entry-card--header">
    @if($entry->is_new())
      ...
      @endif
      <h3 class="entry-card--title"><a href="{{ route(($route ?? 'volunteer.single'), ['entry' => $entry->slug]) }}">{{$entry->title}}</a></h3>
  ...
  <div class="entry-card--footer">
    <div class="entry-card--footer--left">
      <span class="entry-card--hours"><x-icon.clock />{{ $entry->hours ? $entry->hours . ' hrs / week' : 'Flexible hours' }}</span>
      {{-- <span class="entry-card--dates">{{ $entry->date_range() }}</span> --}}
    </div>
    <div class="entry-card--footer--right">
      @if($entry->from_home)
        <span class="entry-card--location"><x-icon.house />From home</span>
      @elseif($entry->address_ward)
        <span class="entry-card--location"><x-icon.map-marker />{{ $entry->address_ward }}</span>
      @endif
      @if($entry->distance)
