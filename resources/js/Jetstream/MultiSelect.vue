...
<div v-click-outside="closeDropdown" class="relative">
    ...
    <div v-if="!expand" class="cursor-pointer relative w-full rounded-md h-12 border border-gray-300 bg-white pl-3 pr-10 py-2 text-left sm:text-sm sm:leading-5" :class="isOpen ? 'z-30 border-indigo-300 ring ring-indigo-200 ring-opacity-50' : ''">
        <div v-if="multiple" class="flex items-center space-x-2">
            <span v-for="v in value" class="relative block truncate flex-shrink px-2 py-1 pr-6 border rounded-full text-blue-800 bg-blue-100">
                <span class="">{{ options.find(o => o[identifier] === v)[label] }}</span>
        ...
        <div v-else class="flex items-center space-x-2">
            <input :tabindex="isOpen ? (-1) : ''" readonly :value="value ? options.find(o => o[identifier] === value)[label] : ''" @focus="openDropdown" class="relative block truncate flex-grow px-2 py-1 pr-6 border-0 p-0 focus:ring-0">
        </div>
