<template>
  <app-layout>
    <template #header>
      ...
      <h2 v-else-if="event" class="text-xl font-semibold leading-tight text-gray-800">
        Clone event
      </h2>
    ...
    <template #actions>
      ...
      <div
        v-else-if="event && event.id && event.status == 'Draft'"
      ...
      <div v-else-if="event && event.id" class="mx-4 text-sm font-semibold">
        Expires on {{ add_days(event.updated_at, 90) }}
      </div>
      <jet-secondary-button
        v-if="isDirty && (!(event && event.id) || event.status == 'Draft')"
      ...
      >
      <jet-secondary-button
        v-else-if="!isDirty && event && event.id && event.status == 'Published'"
        @click.native="saveEntry(false)"
        :class="{ 'opacity-25': form.processing }"
        :disabled="form.processing"
        >Change to draft</jet-secondary-button
      >
      <p class="mx-4 text-sm font-semibold" v-else-if="!isDirty && event && !event.id">
      ...
      <jet-button
        v-if="
          (isDirty && !(event && event.id)) ||
          (event && event.id && event.status == 'Draft')
        "
        class="ml-2"
      ...
      <div class="flex flex-col">
        <form @submit.prevent="form.post('/login')">
          ...
          <jet-fieldset>
            ...
            <template #description>
              Provide contact details for your event. These are optional but will be shown
          ...
          <jet-fieldset>
            ...
            <template #description>
              Let people know when your event takes place, as well as the cost if
              applicable.
            </template>
          ...
          <jet-fieldset>
            ...
            <template #description>
              Provide the location of your event. Letting people know how to get there can
              be helpful.
            </template>
