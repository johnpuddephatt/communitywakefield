<template>
  <app-layout>
    ...
    <template #actions>
      <jet-button :href="route('event.create')">Create event</jet-button>
    </template>

    <div class="flex flex-col">
      <div v-if="events.length" class="-my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
              ...
              <tr v-for="event in events">
                <td class="px-6 py-3 text-center">
                  <label class="form-checkbox">
                    <input
                      ...
                      :value="event.id"
                      v-model="selected"
                      :class="{ 'opacity-20 cursor-not-allowed': !canDelete(event) }"
                      :title="
                        !canDelete(event)
                ...
                <td class="px-6 py-4">
                  ...
                  <div class="text-sm text-gray-500 text-ellipsis">
                    <span v-if="event.subteam">{{ event.subteam.name }}</span>
                  </div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap">
                  <template v-if="event.status == 'Published'">
                    <span
                      v-if="past_expiry(event.updated_at)"
                      class="inline-flex px-2 text-xs font-semibold leading-5 text-red-800 bg-red-100 rounded-full "
                    >
                      Expired
                    </span>
                    <span
                      v-else-if="near_expiry(event.updated_at)"
                ...
                <td class="px-6 py-4 text-sm font-medium text-right whitespace-nowrap">
                  <inertia-link
                    ...
                    :href="route('event.clone', { event: event.id })"
                    >Clone</inertia-link
                  >
                  <a
      ...
      >
        ...
        <p class="relative mt-24 text-gray-400">
          No events have been added to this team yet.
          <a class="text-navy" :href="route('event.create')">Create an event</a>
        </p>
      </div>
    </div>
  </app-layout>
...
export default {
  ...
  methods: {
    ...
    deleteEntries() {
      this.form.delete(route("events.destroy", this.selected.join("-")), {
        preserveScroll: true,
        onSuccess: () => {
