    ...
    <div class="flex flex-col">
              ...
              <tr v-for="course in courses">
                <td class="px-6 py-3 text-center">
                  <label class="form-checkbox">
                    <input
                      ...
                      :value="course.id"
                      v-model="selected"
                      :class="{ 'opacity-20 cursor-not-allowed': !canDelete(course) }"
                      :title="
                        !canDelete(course)
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <template v-if="course.status == 'Published'">
                    <span
                      v-if="past_expiry(course.updated_at)"
                      class="inline-flex px-2 text-xs font-semibold leading-5 text-red-800 bg-red-100 rounded-full "
                    >
                      Expired
                    </span>
                    <span
                      v-else-if="near_expiry(course.updated_at)"
                ...
                <td class="px-6 py-4 text-sm font-medium text-right whitespace-nowrap">
                  <inertia-link
                    ...
                    :href="route('course.clone', { course: course.id })"
                    >Clone</inertia-link
                  >
                  <a
      ...
      >
        ...
        <p class="relative mt-24 text-gray-400">
          No courses have been added to this team yet.
          <a class="text-navy" :href="route('course.create')">Create an course</a>
        </p>
      </div>
    </div>
  </app-layout>
...
export default {
  ...
  methods: {
    select() {
      if (this.selectAll) {
        ...
        this.selected = this.courses
          .filter((course) => this.canDelete(course))
    ...
    canDelete(course) {
      return (
        course.created_by == this.$page.props.user.id ||
