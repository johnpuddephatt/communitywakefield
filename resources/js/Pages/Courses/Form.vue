...
<app-layout>
    <template #header>
        <h2 v-if="course" class="font-semibold text-xl text-gray-800 leading-tight">Edit course</h2>
        <h2 v-else class="font-semibold text-xl text-gray-800 leading-tight">Create course</h2>
    </template>
    ...
    <template #actions>
        ...
        <div v-else-if="(course && course.status == 'Draft')" class="mx-4 text-sm font-semibold"><span class="text-gray-500 mr-1 text-3xl leading-none align-bottom">•</span><span class="align-text-bottom">Saved as draft</span></div>
        <div v-else-if="course" class="mx-4 text-sm font-semibold">Expires on {{ add_days(course.updated_at, 90) }}</div>
        <jet-secondary-button v-if="isDirty && (!course || course.status == 'Draft')" @click.native="saveEntry(false)" :class="{ 'opacity-25': form.processing }" :disabled="form.processing">Save draft</jet-secondary-button>
        <jet-secondary-button v-else-if="!isDirty && course && course.status == 'Published'" @click.native="saveEntry(false)" :class="{ 'opacity-25': form.processing }" :disabled="form.processing">Change to draft</jet-secondary-button>

        <jet-button v-if="isDirty && !course || (course && course.status == 'Draft')" class="ml-2" @click.native="saveEntry(true)" :class="{ 'opacity-25': form.processing }" :disabled="form.processing">Publish</jet-button>
        ...
        <jet-button v-else-if="course" class="ml-2" @click.native="saveEntry(true)" :class="{ 'opacity-25': form.processing }" :disabled="form.processing">Renew</jet-button>
    ...
    <div class="max-w-7xl mx-auto py-10 sm:px-6 lg:px-8">
        <div class="flex flex-col">
            <form @submit.prevent="form.post('/login')">
                ...
                <jet-fieldset>
                    ...
                    <template #form>
                        <div class="col-span-6 sm:col-span-6">
                            <jet-label for="accessibility" value="Who is your course accessible to?" />
                        ...
                        <div class="col-span-6 sm:col-span-4">
                            ...
                            <jet-textarea id="requirements" class="mt-1 block w-full" v-model="form.requirements" />
                        ...
                        <div class="col-span-6 sm:col-span-4">
                            ...
                            <jet-textarea id="qualification" class="mt-1 block w-full" v-model="form.qualification" />
        ...
        <jet-confirmation-modal :show="confirmingRenewal" @close="confirmingRenewal = false">
            ...
            <template #footer>
                ...
                <jet-button class="ml-2" @click.native="saveCourse" :class="{ 'opacity-25': form.processing }" :disabled="form.processing">
                    Confirm renewal
                </jet-button>
            </template>
