...
<app-layout>
    ...
    <div class="flex flex-col">
        <div v-if="services.length" class="-my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr v-for="service in services">
                                <td class="px-6 py-3 text-center">
                                    <label class="form-checkbox">
                                        <input type="checkbox" :value="service.id" v-model="selected">
                                        <i class="form-icon"></i>
                                    </label>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <template v-if="service.status == 'Published'">
                                        <span v-if="past_expiry(service.updated_at)" class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800">
                                            Expired
                                        </span>
                                        <span v-else-if="near_expiry(service.updated_at)" class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-yellow-100 text-yellow-800">
                                            Review now
                                        </span>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <a :href="route('service.edit', service.id)" class="text-indigo-600 hover:text-indigo-900 pr-6">Edit</a>
                                </td>
                            </tr>
                        </tbody>
                    </table>
            ...
            <div v-if="!services.length" class="my-24 flex-1 flex flex-col justify-center items-center">
                ...
                <p class="relative text-gray-400 mt-24">No services have been added to this team yet. <a class="text-navy" :href="route('service.create')">Create a service</a></p>
            </div>
        </div>
</app-layout>
</template>
...
export default {
	...
	methods: {
		select() {
            this.selected = this.selectAll ? [] : this.services.map(o => o.id);
		},
