    ...
    <div class="flex flex-col">
              ...
              <tr v-for="activity in activities">
                <td class="px-6 py-3 text-center">
                  <label class="form-checkbox">
                    <input
                      ...
                      :value="activity.id"
                      v-model="selected"
                      :class="{ 'opacity-20 cursor-not-allowed': !canDelete(activity) }"
                      :title="
                        !canDelete(activity)
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <template v-if="activity.status == 'Published'">
                    <span
                      v-if="past_expiry(activity.updated_at)"
                      class="inline-flex px-2 text-xs font-semibold leading-5 text-red-800 bg-red-100 rounded-full "
                    >
                      Expired
                    </span>
                    <span
                      v-else-if="near_expiry(activity.updated_at)"
                ...
                <td class="px-6 py-4 text-sm font-medium text-right whitespace-nowrap">
                  <inertia-link
                    ...
                    :href="route('activity.clone', { activity: activity.id })"
                    >Clone</inertia-link
                  >
                  <a
      ...
      >
        ...
        <p class="relative mt-24 text-gray-400">
          No activities have been added to this team yet.
          <a class="text-navy" :href="route('activity.create')">Create an activity</a>
        </p>
      </div>
    </div>
  </app-layout>
...
export default {
  ...
  methods: {
    select() {
      if (this.selectAll) {
        ...
        this.selected = this.activities
          .filter((activity) => this.canDelete(activity))
    ...
    canDelete(activity) {
      return (
        activity.created_by == this.$page.props.user.id ||
